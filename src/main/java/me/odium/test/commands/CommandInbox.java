  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (isread == 0) {
          formattedMessageID = Strings.padEnd(
              SimpleMailPlugin.format("&7 [&a%d&7]", messageID), 15, ' ');
        } else {
          formattedMessageID = Strings.padEnd(
              SimpleMailPlugin.format("&7 [%d]", messageID), 11, ' ');
        }
        String msgSender = Strings.padEnd(rs.getString("sender"),
            17, ' ');

        sender.sendMessage(formattedMessageID + " " + msgSender + " "
            + rs.getString("fdate"));
      }

    } catch (ExecutionException e) {
      sender.sendMessage(ChatColor.GRAY + "[SimpleMail] " + ChatColor.RED
          + "An internal error occured while reading your inbox");
