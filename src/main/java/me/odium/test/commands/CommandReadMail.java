...
public class CommandReadMail extends AbstractCommand {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    try {
      ...
      sender.sendMessage(ChatColor.GOLD + "Message Open: "
          + ChatColor.WHITE + rs.getString("id"));
      if (!isSender) {
      ...
      sender.sendMessage(ChatColor.GRAY + " Date: "
          + ChatColor.WHITE + rs.getString("fdate"));
      sender.sendMessage(ChatColor.GRAY + " Expires: " + ChatColor.WHITE + expiration);
      sender.sendMessage(ChatColor.GRAY + " Message: "
          + ChatColor.WHITE + rs.getString("message"));

      if (isTarget && rs.getInt("isread") == 0) {
        service.executeUpdate(Statements.MarkRead,
            plugin.getConfig().getString("MailExpiration"), messageId);
      }
    } catch (ExecutionException e) {
      sender.sendMessage(ChatColor.GRAY + "[SimpleMail] " + ChatColor.RED
          + "An internal error occured while opening the mail");
